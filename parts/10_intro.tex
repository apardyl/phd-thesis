...
\subsection{The Moravec's paradox}
...
the environment is not static, but changes constantly, making it impossible to map and analyse it part by part.
Therefore, the agent must be able to reason based on partial information, while actively pointing its sensors toward most informative regions to gather more data as needed.
...
but also enabled the development of vision-language models~\cite{achiam2023gpt,team2023gemini}, which generalised and scaled up the ability of machines to transcribe images into text and vice versa in an unprecedented scale.
...
\begin{figure}[t]
    ...
    \caption[The two scales of active exploration]{\textbf{The two scales of active exploration.} At the local scale, a stationary agent (e.g. a pan-tilt-zoom camera, or robot's head) remains at a fixed position and redirects its gaze towards different regions of the surrounding scene. At the global scale, a mobile agent (e.g. a robot or a UAV) relocates itself through three-dimensional space.}
...
This allows to use relatively simple object-goal navigation methods~\cite{batra2020objectnav,gu2022vision} to produce strong results on those benchmarks, but it does not address the problem of general exploration.
...
The third contribution, ``AdaGlimpse: Active Visual Exploration with Arbitrary Glimpse Position and Scale''~\pubcite{pardyl2024adaglimpse}, investigates Hypothesis~\ref{hyp:adaglimpse} and closes the gap between
