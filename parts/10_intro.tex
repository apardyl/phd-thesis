...
Moreover, the exploration is often limited to a grid game, where an agent uncovers a fixed area at each move~\cite{rangrej2022consistency},
 a simplification that mirrors the grid input format of vision transformers rather than the continuous, zoomable optics of real
cameras or moving agents. Embodied navigation is most often studied in constrained indoor simulators~\cite{savva2019habitat,kolve2017ai2,xia2018gibson},
...
This allows relativly simple object-goal navigation methods~\cite{batra2020objectnav,gu2022vision} to produce strong results on those benchmarks, but it does not address the problem of general exploration. 
...
local scale, or follows a fairly constrained navigation task: an agent at each step decides where to look or move within a scene it can already sense.
Whether the same efficient, learned decision-making transfers to the unconstrained, zero-shot global-scale search largely remains an open question.
Finally, it is not clear if methods designed for simplified simulation environments can be transferred to real phisical world, bridging the sim-to-real gap.


\section{Learning to explore efficiently}

The central thesis of this dissertation is that \emph{active visual exploration} should be treated as a learning problem in its own right.
Doing so yields agents which are \emph{efficient}, in the sense of understanding an environment from as few observations as possible.
Moreover, they are \emph{flexible}, in the sense of exploiting the true continuous capabilities of real sensors rather than artificial grids.
And finally, they are \emph{scalable} toward the demands of real embodied agents operating in the open world, spanning both the local scale of choosing where to look and the global scale of choosing where to move.
...
The second contribution, ``Beyond Grids: Exploring Elastic Input Sampling for Vision Transformers''~\cite{pardyl2023grids}, removes that constraint.
It formalizes the notion of input \emph{elasticity}, the resilience of a model to patches of varying scale, position, and coverage, introduces a protocol to
measure it, and proposes architectural and training modifications that let a vision transformer ingest patches sampled freely from an image. This turns the
rigid grid into a continuous sampling space, a prerequisite for exploration that matches real optics.
...
The first three papers progressively generalize \emph{local-scale} exploration: deciding
...
environments and uses it to ask whether today's vision-language models, can execute this kind of exploration. The answer is sobering:
even state-of-the-art models fail to reliably solve even simple exploration tasks,
...
{\color{red}TODO: add thesis outline}