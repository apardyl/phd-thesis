...
\end{equation}
where $Q$ and $K$ pack together the query and key projections of all $M$ token embeddings produced by that layer, and $d_k$ is the keys and queries dimension. Row $i$ of $A$ is the probability distribution used to compute a weighted average of all patch representations when producing the updated representation of patch $i$. A peaked row means the model relies on a few already well-understood patches, while a flat row means it must combine evidence from many patches because none of them alone provides a confident answer. Figure~\ref{fig:ame-entropy} illustrates this on a toy $2\times2$ example.
...
\end{equation}
where $A_{h,t}[i]$ is the $i$-th row of head $h$ attention matrix at step $t$, and $H(\cdot)$ is the Shannon entropy of Equation~\ref{eq:shannon-entropy}. Entries corresponding to already-known patches are set to zero so that they are never re-selected, and the next glimpse is placed following a greedy selection strategy, i.e. at the position of maximal remaining entropy,
\begin{equation}
