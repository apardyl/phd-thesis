...
As described in Section~\ref{sec:prelim-pos-enc}, a standard ViT relies on learnable positional embeddings indexed by grid position. This design choice is valid in standard benchmarks where every image is processed by the same fixed (most commonly) $14\times14$ grid of $16\times16$ patches, but it creates a fundamental brittleness in any deployment where the patch positions or sizes deviate from those seen during training. Consider a model trained on a uniform grid: when evaluated with a patch that is $48\times48$ pixels instead of $16\times16$, the patch is bilinearly resampled to the expected input size before being fed to the network. However, its positional embedding still corresponds to a $16\times16$ grid cell, representing a spatial extent that is nine times smaller than the actual patch. The model has no way to distinguish this patch from a normal one at that grid position, yet the two carry fundamentally different information about the scene.

The same problem arises when patches are displaced by even a single pixel from their expected grid positions (e.g. due to imperfect camera alignment). The nearest learnable embedding is used as an approximation, but embeddings are only trained to be meaningful at exact grid locations. In real-world settings, a PTZ camera may not snap to discrete zoom levels, and a UAV may fly at an altitude that does not correspond to any training configuration. More importantly, an active vision system may benefit from being able to select a glimpse at a position not aligned to a pixel grid. This publication asks, as an empirical question, how fragile standard ViTs are under such conditions, and what architectural and training changes are needed to make them robust?
...
forms of input data configurations that can occur in real-life applications. Three distinct types of elasticity are defined, each corresponding to a perturbation commonly encountered in embodied AI applications.

\textbf{Scale elasticity} measures resilience when individual patches are sampled at a scale different from the native $16\times16$ pixel resolution. Each patch $p = (x, y, s)$ is characterised by its top-left grid coordinates $(x, y)$ and a relative scale $s$. The patch covers a $16s \times 16s$ pixel region in the original image, which is then resampled to the standard $16\times16$ input size. A scale of $s = 2$ means the patch covers four times the usual area at half the resolution, $s = 0.5$ covers one quarter the area at twice the resolution, potentially leaving uncovered gaps between adjacent patches. This corresponds to the zoom ability of a camera.

\textbf{Positional elasticity} (also called \emph{patch shake}) measures resilience when patch centres are displaced from their grid positions. The coordinates $(x, y)$ of each patch are independently perturbed by an offset drawn uniformly from $[-r \cdot q,\, r \cdot q]$, where $r$ is the patch size and $q$ is the shake magnitude as a fraction of the patch size. This simulates the non-grid-aligned sampling that arises whenever an image is captured at a position that does not correspond exactly to a discrete training configuration, e.g. due to imperfect camera alignment or agent's drift in real space.

\textbf{Missing-data elasticity} measures resilience when a fraction $d$ of patches are simply removed from the input, leaving the corresponding image regions unobserved. This is the most directly relevant perturbation for active visual exploration, where an agent has only observed a subset of the scene, or lost some parts of the scene due to e.g. occlusion or communication failure.
...
\begin{figure}[H]
    ...
    \begin{tabular}{r@{\hspace{6pt}}c@{\hspace{4pt}}c@{\hspace{4pt}}c@{\hspace{4pt}}c}
        \toprule
        & 50\% & 75\% & 150\% & 200\% \\
        Scale & \includegraphics[width=1.5cm]{papers_src/BeyondGrids/figures/elasticities/scale/50.png}
               ...
               & \includegraphics[width=1.5cm]{papers_src/BeyondGrids/figures/elasticities/scale/200.png} \\[6pt]
        & 12.5\% & 25\% & 37.5\% & 50\% \\
        Position & \includegraphics[width=1.5cm]{papers_src/BeyondGrids/figures/elasticities/position/12.5.png}
                  ...
                  & \includegraphics[width=1.5cm]{papers_src/BeyondGrids/figures/elasticities/position/50.png} \\[6pt]
        & 20\% & 40\% & 60\% & 80\% \\
        Missing data & \includegraphics[width=1.5cm]{papers_src/BeyondGrids/figures/elasticities/dropout/20.png}
    ...
    \end{tabular}
    \end{small}
    \caption[The three types of input elasticity evaluated in BeyondGrids]{\textbf{Elasticity perturbations.} Each row illustrates one type of perturbation applied to an example image at increasing intensity. \emph{Scale}: patches are sampled at sizes ranging from 50\% to 200\% of the native $16\times16$ size; undersampling introduces gaps while oversampling introduces overlap. \emph{Position}: patches are displaced from their grid positions by up to 50\% of the patch size (patch shake). \emph{Missing data}: an increasing fraction of patches is dropped from the input. Adapted from~\cite{pardyl2023grids}.}
    \label{fig:beyond-perturbations}
\end{figure}

The three perturbations are composed into a formal pipeline. For image $I$, an initial patch set $P$ (standard grid, $s=1$, no displacement) is processed by
\begin{equation}
...
\end{equation}
where $\mathcal{E}_{\mathrm{scale}(s_1,s_2)}$ samples each patch's scale independently from $[s_1, s_2]$, $\mathcal{E}_{\mathrm{pos}(q)}$ shifts each patch centre by at most $q$ patch-widths in each direction, and $\mathcal{E}_{\mathrm{miss}(d)}$ removes $d$ patches at random. Elasticity is high if the model's predictions for the perturbed set $P'$ match those for the original $P$.

\subsection{ElasticViT: architecture and training}
...
\begin{axis}[
    ...
    height=3cm,
    width=.85\linewidth,
    legend style={at={(1.6,1.5)},anchor=north,/tikz/every even column/.append style={column sep=0.1cm},font=\tiny},
...
\end{small}
\end{center}
\caption[Elasticity curves for individual perturbations on ImageNet-1k]{\textbf{Elasticity under isolated perturbations} (ImageNet-1k top-1 accuracy). \textbf{(a)}~Scale elasticity: all baselines degrade sharply when patches deviate from scale~1; ElasticViT maintains near-constant accuracy across the measured spectrum and outperforms all baselines at extreme scales. \textbf{(b)}~Missing-data elasticity: ElasticViT surpasses MAE beyond 75\% dropout, despite never being trained with patch dropout. \textbf{(c)}~Positional elasticity: ElasticViT is nearly immune to patch shake and outperforms all but MAE at the largest perturbations. Adapted from~\cite{pardyl2023grids}.}
\label{fig:beyond-elasticity}
\end{figure}

Under standard conditions (centre of each curve, scale~1, no dropout, no shake), ElasticViT trails ViT by $\approx$1.8 pp, this is the accuracy cost of elastic training. However, as perturbation intensity increases, the gap reverses and widens considerably. At 2$\times$ scale, ElasticViT outperforms ViT by $\approx$3.7 pp; at 80\% missing data, it outperforms MAE (the strongest baseline in that regime) by $\approx$1.7 pp. When perturbations are combined, the advantage grows even further: under simultaneous scale and position perturbation at their maxima, ElasticViT leads ViT by nearly 20 percentage points. While standard ViTs overfit to the specific grid configuration used during training, ElasticViT sacrifices a small amount of this overfitting to gain broad-spectrum robustness.

\textbf{Adaptive sampling.} To test the abilities of ElasticViT for visual exploration problems, we propose two simple, non-learnable baseline strategies: CENTRAL (retina-like sampling) and EDGE (edge detection-based sampling). Figure~\ref{fig:beyond-adaptive} shows two such strategies (illustrated in Figure~\ref{fig:beyond-edge-vis}) and their effect on accuracy as a function of the number of input patches.
...
\node [pict] (input) {\includegraphics[width=2cm]{papers_src/BeyondGrids/figures/edge/edges1.png}};
\node [above=2pt of input, font=\scriptsize\scshape] {Input};
\node [pict, right=1cm of input] (central) {\includegraphics[width=2cm]{papers_src/BeyondGrids/figures/edge/central.png}};
\node [above=2pt of central, font=\scriptsize\scshape] {CENTRAL};
\node [pict, below=0.5cm of central] (edge) {\includegraphics[width=2cm]{papers_src/BeyondGrids/figures/edge/edges3.png}};
\node [above=2pt of edge, font=\scriptsize\scshape] {EDGE};
\draw [arrow] (input) -- (central);
\draw [arrow] (input) -- (edge);
\end{tikzpicture}
\caption[CENTRAL and EDGE adaptive patch sampling strategies]{\textbf{Adaptive sampling strategies.} CENTRAL iteratively subdivides patches closest to the image centre into four smaller ones, producing higher resolution in the foveal region (analogous to retinal sampling). EDGE uses a Canny edge detector to concentrate small patches on high-frequency regions. Reproduced from~\cite{pardyl2023grids}.}
...
\begin{axis}[
    ...
    \addlegendentry{ElasticViT, grid}
    ...
    \addlegendentry{ElasticViT, central}
    \addplot[color=forestgreen4416044]
    coordinates {
...
This result is the direct base for the next paper, AdaGlimpse (Section~\ref{sec:adaglimpse}). Elastic input sampling is not merely a robustness property but an active performance advantage at low observation budgets.

\subsection{Contributions}
\label{sec:contributions-beyond}

The publication ``Beyond Grids: Exploring Elastic Input Sampling for Vision Transformers''~\cite{pardyl2023grids} makes the following contributions to the field:

\begin{itemize}
    \item It formalises the notion of \emph{input elasticity} for vision transformers across three dimensions (scale, position, and missing data) and introduces a rigorous evaluation protocol that enables direct comparison of models on each dimension.
    \item It demonstrates empirically that standard ViTs, MAE, Swin-B, and PVT-v2-B5 are all substantially fragile under these perturbations, with accuracy dropping sharply even for moderate scale or dropout levels.
    \item It proposes ElasticViT, combining continuous four-dimensional positional encoding (replacing learnable embeddings) with elastic training augmentations and the novel PatchMix augmentation. ElasticViT achieves near-constant accuracy across the full range of scale and position perturbations, and surpasses MAE on missing-data resilience despite never being trained with patch dropout.
