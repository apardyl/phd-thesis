...
The same problem arises when patches are displaced by even a single pixel from their expected grid positions (e.g. due to impefect camera alignment). The nearest learnable embedding is used as an approximation, but embeddings are only trained to be meaningful at exact grid locations. In real-world settings, a PTZ camera that does not snap to discrete zoom levels, a UAV at an altitude that does not correspond to any training configuration. Whats more important, an active vision system may benefit from being able to select a glimpse at a position not aligned to a pixel grid. This publication asks, as an empirical question, how fragile are standard ViTs under such conditions, and what architectural and training changes are needed to make them robust?
